...
export const createGame_api = async (req: Request, res: Response) => {
  ...
  if (!name || !price || !type_id) {
    return res.status(400).json({
      message:
        "Missing required fields: name, price, and type_id are mandatory.",
    });
  }
  ...
  if (isNaN(numPrice) || isNaN(numTypeId) || numPrice < 0) {
    return res.status(400).json({
      message: "Price and Type ID must be valid non-negative numbers.",
    });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while creating game." });
  }
};
...
export const updateGame_api = async (req: Request, res: Response) => {
  ...
  if (!name || !price || !type_id) {
    return res.status(400).json({
      message:
        "Missing required fields for update: name, price, and type_id are mandatory.",
    });
  }
  const numPrice = Number(price);
  const numTypeId = Number(type_id);
  ...
  try {
    ...
    if (results.affectedRows === 0) {
      return res
        .status(404)
        .json({ message: "Game not found or no changes made." });
    }
    ...
    return res
      .status(500)
      .json({ message: "Server error while updating game." });
  }
};
...
export const deleteGame_api = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while deleting game." });
  }
};
...
export const createGameType_api = async (req: Request, res: Response) => {
  ...
  if (!typename) {
    return res
      .status(400)
      .json({ message: "Missing required field: typename is mandatory." });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while creating game type." });
  }
};
...
export const createDiscountCode_api = async (req: Request, res: Response) => {
  ...
  if (!code_name || discount_value === undefined || max_user === undefined) {
    return res.status(400).json({
      message:
        "Missing required fields: code_name, discount_value, and max_user are mandatory.",
    });
  }
  ...
  if (
    ...
    return res.status(400).json({
      message:
        "Discount value must be a positive number, and Max User must be a positive integer.",
    });
  }
  ...
  try {
    ...
    if (existing.length > 0) {
      return res
        .status(409)
        .json({ message: `Discount code '${code_name}' already exists.` });
    }
    ...
    return res
      .status(500)
      .json({ message: "Server error while creating discount code." });
  }
};
...
export const getAllGameTypes_api = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while fetching game types." });
  }
};
...
export const getLatestGames_api = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while fetching latest games." });
  }
};
...
export const getTopSellerGames_api = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while fetching top seller games." });
  }
};
...
export const searchGames_api = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while searching games." });
  }
};
...
export const getGameDetails_api = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while fetching game details." });
  }
};
...
export const getAllGames_api = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while fetching all games." });
  }
};
...
export const addToBasket_api = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(201).json({
      message: "Game added to basket successfully.",
      bid: results.insertId,
    });
  } catch (error) {
    console.error(
      `Error adding game ${game_id} to basket for user ${uid}:`,
      error
    );
    return res
      .status(500)
      .json({ message: "Server error while adding to basket." });
  }
};
...
export const removeFromBasket_api = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (results.affectedRows === 0) {
      return res
        .status(404)
        .json({ message: "Basket item not found or does not belong to user." });
    }
...
export const purchaseGame_api = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (basketRows.length === 0) {
      ...
      return res
        .status(200)
        .json({ message: "Basket is empty. No purchase made." });
    }
    ...
    if (code_name) {
      ...
      if (usageCheck.length > 0) {
        ...
        return res
          .status(409)
          .json({ message: "You have already used this discount code." });
      }
    ...
    await dbcon_transaction.query("DELETE FROM basket WHERE uid = ?", [uid]);
    ...
    return res
      .status(500)
      .json({ message: "Server error during purchase process." });
  } finally {
    if (dbcon_transaction) {
...
export const applyDiscount_api = async (req: Request, res: Response) => {
  ...
  if (!code_name || !uid) {
    return res
      .status(400)
      .json({ message: "Missing required fields: code_name and user_id." });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while applying discount." });
  }
};
 ...
 */
export const getTransactionHistory_api = async (
  req: Request,
  res: Response
) => {
  const uid = req.params.user_id;
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while fetching transaction history." });
  }
};
 ...
 */
export const getGamePurchaseHistory_api = async (
  req: Request,
  res: Response
) => {
  const uid = req.params.user_id;
  ...
  try {
    ...
    console.error(
      `Error fetching game purchase history for user ${uid}:`,
      error
    );
    return res
      .status(500)
